...
try:
    import fcntl
except ImportError:  # Windows: emulate byte-range locks with msvcrt
    fcntl = None
    import msvcrt
...
def _pread(descriptor: int, count: int, offset: int) -> bytes:
    ...
    cursor = os.lseek(descriptor, 0, os.SEEK_CUR)
    try:
...
def _pwrite(descriptor: int, payload: bytes, offset: int) -> int:
    if hasattr(os, "pwrite"):
        return os.pwrite(descriptor, payload, offset)
    cursor = os.lseek(descriptor, 0, os.SEEK_CUR)
    try:
        os.lseek(descriptor, offset, os.SEEK_SET)
...
if fcntl is not None:
    def _lock_byte_range(descriptor: int, count: int) -> None:
        fcntl.flock(descriptor, fcntl.LOCK_EX)

    def _unlock_byte_range(descriptor: int, count: int) -> None:
        fcntl.flock(descriptor, fcntl.LOCK_UN)
else:
    def _lock_byte_range(descriptor: int, count: int) -> None:
        os.lseek(descriptor, 0, os.SEEK_SET)
        msvcrt.locking(descriptor, msvcrt.LK_LOCK, count)

    def _unlock_byte_range(descriptor: int, count: int) -> None:
        os.lseek(descriptor, 0, os.SEEK_SET)
        msvcrt.locking(descriptor, msvcrt.LK_UNLCK, count)
...
class SharedMemoryReservationLedger:
    ...
    def __init__(self, path: str | Path, *, reset: bool = False) -> None:
        ...
        self._descriptor = os.open(
            self.path,
            # Windows: force binary mode. Text mode collapses 0x0D 0x0A and
            # stops at 0x1A, so packed counters would read back short and
            # poison the governor with a permanent "truncated" ledger error.
            os.O_RDWR
            | os.O_CREAT
    ...
    def _locked(self):
        ...
        class _Lock:
            def __enter__(self):
                _lock_byte_range(ledger._descriptor, _SHARED_RESERVATION_BYTES)
                return ledger

            def __exit__(self, *_exc_info: object) -> None:
                _unlock_byte_range(ledger._descriptor, _SHARED_RESERVATION_BYTES)
    ...
    def _write_locked(self, values: list[int]) -> None:
        ...
        if _pwrite(self._descriptor, payload, 0) != len(payload):
