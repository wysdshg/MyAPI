...
async def update_stats(current_info):
    # 账号级 token 窗口按实际 usage 累计：必须在 DISABLE_DATABASE 早退
    # 之前，额度限制不能依赖数据库开关。
    try:
        record_response_usage(current_info)
    except Exception as exc:
        logger.warning("record_response_usage failed: %s", exc)
...
async def process_request(
    ...
    if isinstance(current_info, dict):
        ...
        # 账号 token 窗口记账需要上游 Key（update_stats 时按渠道+Key 查额度）
        current_info["provider_api_key"] = provider_api_key_raw
