...
class RoutingPlan:
    ...
    async def next_provider(self) -> Optional[ProviderAttempt]:
        ...
        while self.index < self.num_matching_providers + self.retry_count:
            ...
            pool = provider_api_circular_list[provider_name]
            if await pool.is_all_rate_limited(original_model):
                # 额度/token 窗口耗尽时给出更具体的原因（测试替身没有该方法）。
                reason_fn = getattr(pool, "quota_block_reason", None)
                quota_reason = reason_fn(original_model) if reason_fn is not None else None
                self.record_failure(
                    429,
                    quota_reason or "All API keys are rate limited and stop auto retry!",
                )
