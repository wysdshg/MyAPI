...
def test_routing_plan_records_quota_reason_for_all_keys_exhausted(monkeypatch):
    provider_name = "provider-quota"
    quota_message = "Daily quota exhausted for provider-quota (250/250), resets at midnight"
    ...
    class _QuotaKeys:
        ...
        def quota_block_reason(self, model):
            _ = model
            return quota_message

    monkeypatch.setitem(provider_api_circular_list, provider_name, _QuotaKeys())
        ...
        return [
            {
                "provider": provider_name,
                "_model_dict_cache": {request_model_name: request_model_name},
                "base_url": "https://provider-quota.example/v1/chat/completions",
                "api": ["key-a"],
                "preferences": {},
            }
        ]
    ...
    async def run():
        ...
        assert await plan.next_provider() is None
        assert plan.status_code == 429
        # 额度耗尽时 429 带具体原因，而不是笼统的“全部限速”
        assert plan.error_message == quota_message
