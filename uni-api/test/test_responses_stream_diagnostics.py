...
def test_kernel_socket_unread_sample_revalidates_fd_inode(monkeypatch):
    if responses_stream_observability.fcntl is None:
        pytest.skip("kernel socket sampling needs fcntl (Linux only)")
    ioctl_calls = []
...
def test_exception_chain_errno_and_httpcore_reactive_close_order_are_preserved():
    ...
    # Windows maps ECONNRESET to 10054 and errorcode[10054] is
    # "WSAECONNRESET"; derive the expected name from this platform.
    assert facts["exception_errno_name"] == errno.errorcode[errno.ECONNRESET]
